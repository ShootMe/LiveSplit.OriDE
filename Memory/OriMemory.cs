...
namespace LiveSplit.OriDE.Memory {
    public partial class OriMemory {
        ...
        private static ProgramPointer UberPool = new ProgramPointer(AutoDeref.Single, new ProgramSignature(PointerVersion.V1, "558BEC53575683EC0C8B7D088B05????????83EC0C50E8", 14));
        ...
        public int CurrentEnergyShardCount() {
            if (UberPool.GetPointer(Program) != IntPtr.Zero) {
                //UberPoolManager.Instance.m_groups
                IntPtr listHead = (IntPtr)UberPool.Read<uint>(Program, 0x0, 0x18);
                int listSize = Program.Read<int>(listHead, 0xc);
                byte[] listPtrs = Program.Read(listHead, listSize * 4, 0x8, 0x10);

                for (int i = 0; i < listSize; i++) {
                    IntPtr uberGroup = (IntPtr)BitConverter.ToUInt32(listPtrs, i * 4);
                    string groupName = Program.Read((IntPtr)Program.Read<uint>(uberGroup, 0x18));
                    if (groupName.Equals("energyDropPickup", StringComparison.OrdinalIgnoreCase)) {
                        listHead = (IntPtr)Program.Read<uint>(uberGroup, 0x14);
                        listSize = Program.Read<int>(listHead, 0xc);
                        listPtrs = Program.Read(listHead, listSize * 4, 0x8, 0x10);

                        int count = 0;
                        for (i = 0; i < listSize; i++) {
                            IntPtr uberItem = (IntPtr)BitConverter.ToUInt32(listPtrs, i * 4);
                            bool isDestroyed = Program.Read<bool>(uberItem, 0x24);
                            if (!isDestroyed) { count++; }
                        }
                        return count;
                    }
                }
