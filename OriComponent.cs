    ...
    public class OriComponent : IComponent {
        ...
        public OriComponent(LiveSplitState state) {
            try {
                ...
                energyDisplay = new LayoutComponent("LiveSplit.OriDE.dll", new OriEnergyShardDisplayComponent(mem));
            ...
            try {
                ...
                for (int i = components.Count - 1; i >= 0; i--) {
                    ...
                    if (component.Component is OriMapDisplayComponent) {
                        ...
                        hasMapDisplay = true;
                    } else if (component.Component is OriEnergyShardDisplayComponent) {
                        if (!settings.EnergyShardDisplay || hasEnergyDisplay) {
                            components.Remove(component);
                        }
                        ...
                        if (!hasEnergyDisplay && settings.EnergyShardDisplay) {
                            components.Insert(i, energyDisplay);
                            hasEnergyDisplay = true;
                        }
                    }
                }
